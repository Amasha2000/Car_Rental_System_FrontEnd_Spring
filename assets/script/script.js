...
function loadAllVehicles() {
    $.ajax({
        url: 'http://localhost:8080/EasyCarRental_Server-1.0.0/vehicle/available_vehicles',
        method: 'get',
        async: true,
        dataType: "json",
        success: function (response) {
            ...
            for (let i = 0; i < vehicle_list.length; i++) {
                ...
                //load luxury cars
                if (vehicle_list[i].type.toLowerCase() === 'luxary') {
                    if (vehicle_list[i].vehicleDetailList.length > 0) {
...
function setVehicleImage(image, div, div_index) {
    $.ajax({
        type: "GET",
        url: "http://localhost:8080/EasyCarRental_Server-1.0.0/vehicle_detail/get_images/" + image,
...
function addUser(user_role, obj, user_nic_image, user_nic_image_name, user_dr_license_image, user_dr_license_image_name) {
    $.ajax({
        url: 'http://localhost:8080/EasyCarRental_Server-1.0.0/login/' + user_role,
        method: 'post',
        async: true,
...
function uploadUserNicAndLicenseImage(path, image, fileName) {
    ...
    $.ajax({
        url: 'http://localhost:8080/EasyCarRental_Server-1.0.0/' + path,
        method: 'post',
        async: true,
        processData: false,
        contentType: false,
...
function generateUserId() {
    $.ajax({
        url: 'http://localhost:8080/EasyCarRental_Server-1.0.0/login/lastid',
        method: 'get',
        async: true,
        dataType: 'json',
        success: function (response) {
...
function generateCustomerId() {
    $.ajax({
        url: 'http://localhost:8080/EasyCarRental_Server-1.0.0/customer/lastid',
        method: 'get',
        async: true,
        dataType: 'json',
        success: function (response) {
...
function generateDriverId() {
    $.ajax({
        url: 'http://localhost:8080/EasyCarRental_Server-1.0.0/driver/lastid',
        method: 'get',
        async: true,
        dataType: 'json',
        success: function (response) {
...
function setPickupDate(id) {
    ...
    //get qty
    let qty = $(`#${parent}`).children('span').eq(0).text();
    //get daily rate fee
    let daily_rate_fee = $(`#${parent}`).children('p').eq(5).children('span').eq(0).text();
    ...
    let new_rental_fee;
    if (date_diff_in_days > 0) {
        new_rental_fee = (daily_rate_fee * date_diff_in_days);
        console.log(new_rental_fee);
    } else if (date_diff_in_days === 0) {
        new_rental_fee = (daily_rate_fee);
    } else {
        new_rental_fee = 0;
    }
...
function addVehicleToCart(id) {
    ...
    if (isUserLogged==true) {
        ...
        var isAvailable; 
        var availableOrNot='Available'    

        $.ajax({
            url: 'http://localhost:8080/EasyCarRental_Server-1.0.0/vehicle_detail/get/'+ct_id,
            method: 'get',
            async: true,
            dataType: 'json',
            success: function (response) {
               isAvailable=response.data;
               if(isAvailable==availableOrNot){
...
function uploadBankSlip(image, fileName) {
    ...
    $.ajax({
        url: 'http://localhost:8080/EasyCarRental_Server-1.0.0/request/upload_bank_slip',
        method: 'post',
        async: true,
        processData: false,
        contentType: false,
...
function getNotifications(cid) {
    $.ajax({
        url: 'http://localhost:8080/EasyCarRental_Server-1.0.0/request/get_request',
        method: 'get',
        async: true,
        dataType: 'json',
        data: {
...
function generateRequestID() {
    $.ajax({
        url: 'http://localhost:8080/EasyCarRental_Server-1.0.0/request/lastrid',
        method: 'get',
        async: true,
        dataType: 'json',
        success: function (response) {
...
$('#btn-sign-in').click(function () {
    ...
    $.ajax({
        url: 'http://localhost:8080/EasyCarRental_Server-1.0.0/login',
        method: 'get',
        async: true,
        data: {
            email: email,
        ...
        success: function (response) {
            try {
                if (response.data.role === 'admin') {
                    window.location.replace('views/dashboards/admin/index.html');
                } else if (response.data.role === 'customer') {
                 
                    //get customer data
                    $.ajax({
                        url: 'http://localhost:8080/EasyCarRental_Server-1.0.0/customer/' + response.data.uid,
                        method: 'get',
                        dataType: 'json',
                        success: function (response2) {

                            $('#customerForm').css('display','flex');
                    ...
                    window.location.replace('views/dashboards/driver/index.html');
                }
            } catch (e) {
